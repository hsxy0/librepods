...
`Tap to reconnect` 由实际 AACP 套接字状态决定。上述日志显示 L2CAP 模式协商失败，不能用界面标志绕过，否则设置项会出现却无法控制耳机。此前认为只能从系统蓝牙栈和设备固件方向继续排查；后续比对兼容提交发现，项目已有处理此问题的原生 Hook，而合并时新增的进程名检查可能使 Hook 提前退出。详见 [后续修复报告](20260924-AirPods5-L2CAP-Hook回归.md)。
